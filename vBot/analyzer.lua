...
analyzerButton = modules.game_buttons.buttonsWindow.contentsPanel and
    modules.game_buttons.buttonsWindow.contentsPanel.buttons.botAnalyzersButton
...
--button
analyzerButton = modules.client_topmenu.addRightGameToggleButton('botAnalyzersButton', 'vBot Analyzers',
  '/images/topbuttons/analyzers', toggle, false, 998)
analyzerButton:setOn(false)
...
local function createTrackedItems()
  ...
  for k, v in pairs(trackedLoot) do
    ...
    local id = tonumber(k)
    local itemName = id == 3031 and "gold coin" or id == 3035 and "platinum coin" or id == 3043 and "crystal coin" or
        Item.create(id):getMarketData().name
...
if BotServer._websocket then
  BotServer.listen("partyHunt", function(name, message)
    ...
    elseif message == false then
      ...
      local widget = partyHuntWindow.contentsPanel[widgetName] or
          UI.createWidget("MemberWidget", partyHuntWindow.contentsPanel)
...
onTextMessage(function(mode, text)
  ...
  if re ~= 0 then
    for i = 1, #re do
      ...
      local price = amount and getPrice(formattedLoot) * amount or
          getPrice(formattedLoot)                        -- if amount then multity price, else just take price
...
function refreshLoot()
  ...
  for k, v in pairs(lootedItems) do
    ...
    label1:setTooltip(v.count ..
      "x " .. tooltipName .. " (Value: " .. format_thousand(getPrice(v.name)) .. "gp, Sum: " ..
      format_thousand(price) .. "gp)")
...
function refreshWaste()
  ...
  for k, v in pairs(usedItems) do
    for i = 1, #parents do
      ...
      label1:setTooltip(amount ..
        "x " .. tooltipName .. " (Value: " .. format_thousand(getPrice(v.name)) ..
        "gp, Sum: " .. format_thousand(price) .. "gp)")
    end
  end
end
...
onAddItem(function(container, slot, item, oldItem)
  ...
  local tmpname = item:getId() == 3031 and "gold coin" or item:getId() == 3035 and "platinum coin" or
      item:getId() == 3043 and "crystal coin" or item:getMarketData().name
...
-- ammo
local ammo = { 16143, 763, 761, 7365, 3448, 762, 21470, 7364, 14251, 3447, 3449, 15793, 25757, 774, 35901, 6528, 7363,
  3450, 16141, 25758, 14252, 3446, 16142, 35902 }
onContainerUpdateItem(function(container, slot, item, oldItem)
  local id = item:getId()
...
function reportStats()
  ...
  a = "Session Time: " ..
      sessionTime() .. ", Exp Gained: " .. format_thousand(expGained()) .. ", Exp/h: " .. expPerHour()
